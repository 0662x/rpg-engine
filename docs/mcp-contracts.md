...
- `player` profile 不能注册 `intent_preflight`、`start_turn`、`query`、`preview_from_text`、
  `preview_action`、`validate_delta` 或 `commit_turn`。
...
| `save_current` | 返回 active save；registry 只是索引，不是事实源。 |
...
| `intent_manifest` | 只读 action/query/slot 合同；不是玩法入口。 |
...
| `save_inspect` | 只读检查 configured save package。 |
