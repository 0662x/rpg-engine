...
- **AI Intent Candidate**：由外部 AI、内部 AI、后台 AI 或 fallback logic 提供的低信任玩家意图解释。`low-trust` 限定的是事实、权限、玩家确认和提交边界，不表示 deterministic rules 永远拥有路由否决权；具体 route selection 由显式 internal intent AI mode 决定。当前实现中相关名字包括 `external_intent_candidate`、`IntentCandidate`、`AIIntentRouter`、`ActionIntent` 和 `intent_preflight_cache`。
...
- **AI-first interpretation, kernel-enforced facts。** 自然语言理解应主要由 AI 完成。路由选择按 internal intent AI mode 决定：当 internal intent AI 明确关闭且存在通过 kernel 结构、安全和绑定检查的 external candidate 时，external candidate 是主路由提案，deterministic rules 仅记录 trace / diagnostic evidence，不得改写、否决或仅因 mismatch 强制 clarification；当 internal intent AI 开启时，保持 external / internal 仲裁流程；当 internal intent AI 关闭且没有 external candidate 时，保持当前 deterministic fallback。以上任何路由结果都不授予事实、hidden access、玩家确认、proposal approval 或 commit 权限。
...
引擎必须能接收外部或 resident AI intent candidates，并按显式 internal intent AI mode 选择 intent route。`low-trust` 表示 candidate 不能成为事实、权限、玩家确认或保存授权；它不表示一个通过 kernel 结构、安全和绑定检查的 external candidate 在 internal intent AI 关闭时仍必须服从 deterministic rules。
...
- 当 internal intent AI 关闭且存在合法 external candidate 时，external candidate 是 proposal authority；若 external 与 rules 冲突，rules 仅记录 trace，不得 override、veto 或仅因 mismatch 强制 clarification。
- 当 internal intent AI 开启时，必须保持 external / internal 仲裁流程；external candidate 不因存在而无条件成为 proposal authority。
- 当 internal intent AI 关闭且没有 external candidate 时，引擎保持当前 deterministic rules fallback；rule matching 不是产品的主 NLU 策略。
...
- **NFR-1 Safety:** AI 输出在通过 kernel 的结构、风险、安全和绑定检查前，不是 selected route；无论是否被选为 route proposal，都永远不是事实、hidden access、player confirmation、proposal approval 或 save authorization。
